...
export async function Header() {
  ...
  const navLinkClass =
    "inline-flex items-center rounded-xl px-4 py-2 text-sm font-medium text-slate-300 transition-colors duration-200 hover:bg-white/5 hover:text-amber-500 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-700/60"
  const subtleAuthButtonClass =
    "rounded-xl px-5 py-2 text-sm font-medium text-slate-100 transition-colors hover:bg-white/5 hover:text-amber-500"

  return (
    <header className="fixed left-1/2 top-6 z-50 w-[calc(100%-2rem)] max-w-7xl -translate-x-1/2 rounded-2xl border border-white/5 bg-slate-950/50 shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] backdrop-blur-3xl transition-all duration-300 hover:bg-slate-950/70 hover:shadow-[0_8px_32px_0_rgba(0,0,0,0.5)]">
            ...
            {user ? (
              ...
              <>
                ...
                <Link href="/auth/signup">
                  <Button
                    size="sm"
                    className="group relative overflow-hidden rounded-xl border border-blue-500/30 bg-blue-500/10 px-6 text-sm font-semibold text-blue-200 shadow-[0_0_20px_-5px_rgba(59,130,246,0.3)] hover:bg-blue-500/20 hover:shadow-[0_0_25px_-5px_rgba(59,130,246,0.5)] hover:border-blue-500/50"
                  >
